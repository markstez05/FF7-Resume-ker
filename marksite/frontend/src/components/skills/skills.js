...
import { deleteSkill, getSkill, addSkill, getSkillById } from '../../actions/SkillActions';
...
class Skills extends Component {
    ...
    render() {
        ...
        const { title, desc, level, type } = this.state;
        return <div className="experience_list">
            <div className="exp_menu">
                ...
                <div id="data" className="slots">
                    ...
                    <div className="skill-bar">
                        <div id="data" onDrop={this.drop} onDragOver={this.allowDrop} className="circles" />
                    ...
                    <div className="work_form2">
                        <div className="work_form1">
                            {skills.map((skill, i) => {
                                ...
                                return <SkillButton key={_id} id={_id} index={i} title={title} desc={desc} level={level} type={type} onSkillClick={() => this.onSkillClick(_id)} drag={this.drag} drop={this.drop} />;
                            })}
                        </div>
                        <div className="exp_bar2">
                            ...
                            <div className="work_input">
                                <form onSubmit={this.submit}>
                                    <div className="input">
                                        <input type="text" placeholder="Skill Title" value={title} onChange={e => this.setState(
                                            { title: e.target.value }
                                        )} />
                                    </div>
                                    <div className="input">
                                        <input type="number" placeholder="Skill Level 1-5" value={level} onChange={e => this.setState(
                                            { level: e.target.value }
                                        )} />
                                    </div>
                                    <div className="input">
                                        <select className="select" type="text" placeholder="Skill Type" value={type} onChange={e => this.setState(
                                            { type: e.target.value }
                                        )}>
                                            <option value={null} />
                                    ...
                                    <div className="input">
                                        <textarea type="text" placeholder="Skill Description" value={desc} onChange={e => this.setState(
                                            { desc: e.target.value }
                                        )} />
                                        <button className="equip_button">
                                            GAIN SKILL
                          </button>
                                    </div>
                                    <div className="input" />
